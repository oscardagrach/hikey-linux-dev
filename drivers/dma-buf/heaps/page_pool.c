...
static void dmabuf_page_pool_add_clean(struct dmabuf_page_pool *pool, struct page *page)
{
	int index;
...
void dmabuf_page_pool_free(struct dmabuf_page_pool *pool, struct page *page)
{
	...
	dmabuf_page_pool_add_clean(pool, page);
}
						  ...
						  struct shrink_control *sc)
{
	...
	return dmabuf_page_pool_shrink(sc->gfp_mask, to_scan);
}
